...
*Debug.Log 写多了，总会想有个开关能一键关掉它们。DebugTool 就是那个开关。*
...
**Log** - 对应 Debug.Log，用于普通信息
**LogWarning** - 对应 Debug.LogWarning，用于警告信息
**LogError** - 对应 Debug.LogError，用于错误信息
...
1. **发布前关闭日志**：正式发布前记得在 Setting 中关闭日志开关，避免影响性能

2. **错误日志建议保留**：即使发布后也建议保留错误日志，便于排查线上问题

3. **不要过度使用**：日志过多会影响性能，关键位置记录即可
