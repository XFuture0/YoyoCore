...
*有时候，你不是不想继承 MonoBehaviour，只是不想每个类都继承它。*
...
MonoMgr 提供了一个**集中式的事件订阅机制**，让任何类都可以「借用」MonoBehaviour 的生命周期函数。
...
**非常重要**：当对象被销毁或不再需要更新时，一定要取消订阅，否则会造成内存泄漏。
...
MonoMgr 内部使用了 C# 的 **event** 关键字来实现事件订阅：
...
1. **必须取消订阅**：忘记 Remove 会导致内存泄漏，被订阅的方法会一直被引用，无法被垃圾回收

2. **线程安全**：事件回调是在 Unity 主线程中执行的，所以不用担心线程安全问题

3. **性能考虑**：如果有很多对象订阅了 Update，可能会影响性能。对于大量同类对象的更新，考虑使用对象池统一管理

## 与直接继承 MonoBehaviour 的对比
...
*说到底，这就是一种「委托」的思想——把 Update 的工作委托给专门的管家，自己专注于业务逻辑就好。*
